<template>
    <v-col style="height:100%" class="mx-3 d-flex flex-column my-1">
        <v-card class="d-flex flex-column buysellcard" fill-height style="height:95%">
            ...
            <v-card-text class="overflow-y-auto" style="margin-bottom:48px">
                <v-list class="listouter1">
                    <v-list-item-group v-model="selectedSellingBid" active-class="border" color="indigo" class="listouter2">
                        <div id="inner">
                            <v-list-item :disabled="item.trader == $store.state.player_id" v-for="(item, i) in bids" :key="i" :id="`li_${item}`" dense>
                                <v-list-item-content>
